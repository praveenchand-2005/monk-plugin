...
export default function Home() {
  ...
  async function start() {
    ...
    try {
      const demoMode = organizationId === 'demo-org';
      let active = selected;
      if (!active && !demoMode) {
      ...
      if (demoMode) {
        const demoCase: CaseSummary = {
          id: result.caseId,
          name: caseName || `Investigation: ${target}`,
          status: 'IN_PROGRESS',
          targets: [{ id: 'demo-target', kind: targetKind, value: target }],
          createdAt: new Date().toISOString(),
          updatedAt: new Date().toISOString(),
        };
        setSelected(demoCase);
        setCases([demoCase]);
      } else if (active) {
  ...
  return (
    <main className="shell">
      <aside className="sidebar">
        ...
        <div className="navGroup">{['Overview', 'Investigations', 'Entities', 'Evidence', 'Monitoring', 'Reports', 'Connectors', 'Audit'].map((item) => <button key={item} className={tab === item ? 'navItem active' : 'navItem'} onClick={() => { if (item === 'Evidence') setTab('Evidence'); if (item === 'Overview') setTab('Overview'); }}>{item}</button>)}</div>
